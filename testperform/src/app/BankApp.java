...
public class BankApp {
	...
	public void createAccount() {
		...
		try {
			...
			if (balance < 0) {
				System.out.println("결과: 초기 입금액은 0 이상이어야 합니다.");
				System.out.println("------------------------------------------------------");
				return;
			}
		} catch (NumberFormatException e) {
			System.out.println("잘못된 입력입니다. 초기 입금액은 숫자로 입력해주세요.");
			System.out.println("------------------------------------------------------");
			return;
		}
	...
	public void depositAccount() {
		...
		if (account == null) {
			...
			try {
				...
				if (amount <= 0) {
					System.out.println("결과: 0원 이하는 입금할 수 없습니다.");
			...
			System.out.println("결과: 예금이 완료되었습니다. 현재 잔고: " + account.getBalance());
		}
		System.out.println("------------------------------------------------------");
	}
	...
	public void withdrawAccount() {
		...
		if (account == null) {
			...
			try {
				amount = Integer.parseInt(scanner.nextLine());
				if (amount <= 0) {
					System.out.println("결과: 0원 이하는 출금할 수 없습니다.");
					System.out.println("현재 잔고: " + account.getBalance());
					System.out.println("------------------------------------------------------");
					return;
				}
			...
			if (account.getBalance() < amount) { // 잔액 부족 확인
				System.out.println("결과: 잔액이 부족합니다. 현재 잔고: " + account.getBalance());
			} else {
				account.setBalance(account.getBalance() - amount); // 잔고 감소
				System.out.println("결과: 출금이 완료되었습니다. 현재 잔고: " + account.getBalance());
			}
		}
		System.out.println("------------------------------------------------------");
	}
